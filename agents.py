...
# Groq enforces a tokens-per-minute limit for the selected model.
# Tool/MCP responses can be much larger than the useful information needed
# by downstream agents, so keep prompts compact before sending them to the LLM.
def _compact(value: Any, max_chars: int = 3000) -> str:
    if value is None:
        return ""
    ...
    if len(text) <= max_chars:
        return text
    # Preserve both the beginning and end because API/tool responses often
    # put summaries at the beginning and totals/important details at the end.
    head = max_chars * 2 // 3
    tail = max_chars - head
    return text[:head] + "\n...[content shortened]...\n" + text[-tail:]
...
def itinerary_agent(state: TravelState):
    ...
    # Keep the itinerary prompt well below Groq's TPM/request limit.
    # MCP search results can contain thousands of tokens, while the itinerary
    # agent only needs a compact summary of each specialist's findings.
    prompt = f"""
Create a clear draft travel itinerary.

User request:
{_compact(state.get('user_query', ''), 1800)}

Trip constraints:
{_compact(state.get('trip_constraints', {}), 1800)}

Flight results:
{_compact(state.get('flight_results', ''), 2800)}

Hotel results:
{_compact(state.get('hotel_results', ''), 2800)}

Weather results:
{_compact(state.get('weather_results', ''), 2200)}

Budget results:
{_compact(state.get('budget_results', ''), 2200)}

Use only the useful facts provided above. Do not invent live prices,
flight schedules, hotel availability, or weather data. Make the output
structured, practical, concise, and ready for human review.
"""
